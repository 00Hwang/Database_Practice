<?php
	...
	$sql = "select link from HB_PRODUCT_IMAGE where id = '$PRODUCT_NUMBER'";
	$stid = oci_parse($conn, $sql); 
	oci_execute($stid);
	$row = oci_fetch_array($stid, OCI_ASSOC+OCI_RETURN_NULLS);
	
	$file_path = $row["LINK"]; // 삭제할 파일의 경로
	...
	$sql = "UPDATE HB_PRODUCT_IMAGE SET 
	link = '$link'
	where product_number = '$PRODUCT_NUMBER'";
	$stid = oci_parse($conn, $sql); 
	oci_execute($stid);
	echo("
			<script> alert('대표사진이 변경되었습니다.'); </script>
